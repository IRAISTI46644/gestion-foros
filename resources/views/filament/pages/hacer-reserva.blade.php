                ...
                @foreach(\App\Models\Foro::all() as $foro)
                    <button wire:click="seleccionarForo({{ $foro->id }})" 
                        ...
                        {{ $selectedForoId == $foro->id 
                            ? 'bg-[#fff5f5] dark:bg-[#2d1212] border-[#800020] ring-1 ring-[#800020] scale-[1.02] shadow-md' 
                            : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 hover:border-[#800020]/30 hover:shadow-md' }}">
                        
                        <div class="mb-3 p-4 rounded-full transition-colors duration-300 {{ $selectedForoId == $foro->id ? 'bg-[#800020] text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 group-hover:bg-[#800020]/10 group-hover:text-[#800020]' }}">
                        ...
                        <div class="text-center">
                            <span class="block font-black text-sm uppercase tracking-tight {{ $selectedForoId == $foro->id ? 'text-[#800020] dark:text-[#f87171]' : 'text-gray-900 dark:text-gray-100' }}">
                                {{ $foro->nombre }}
                            </span>
                    ...
                    <div class="border-b border-gray-100 dark:border-gray-800 pb-6 mb-8">
                        ...
                        <div class="flex items-center gap-2 mt-3">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 text-[10px] font-black uppercase tracking-widest">
                                Seleccionado: <span class="text-[#800020] dark:text-[#f87171] font-bold">{{ $foroNombre }}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div class="space-y-4">
                            ...
                            <input type="date" wire:model.live="selectedDate" 
                                class="w-full bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 border rounded-2xl p-5 text-gray-900 dark:text-white font-black text-xl focus:ring-2 focus:ring-[#800020] focus:border-[#800020] outline-none transition-all shadow-sm">
                        </div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                @foreach($horarios as $hora)
                                    ...
                                    <button 
                                        ...
                                        {{ $isOcupado 
                                            ? 'bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-300 dark:text-gray-600 cursor-not-allowed opacity-50' 
                                            : ($selectedTime == $hora 
                                                ? 'bg-[#800020] border-[#800020] text-white shadow-lg scale-105' 
                                                : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:border-[#800020] hover:text-[#800020]') 
                                        }}">
                                        {{ $hora }}
                                    </button>
                                @endforeach
                        ...
                        @if($selectedDate && $selectedTime)
                            <div class="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 p-8 rounded-[2rem] shadow-inner flex flex-col md:flex-row items-center justify-between gap-6 animate-in fade-in slide-in-from-bottom-4">
                                <div class="text-center md:text-left">
                                    ...
                                    <p class="text-gray-600 dark:text-gray-400 font-bold text-sm mt-1 uppercase">
